...
# База знаний
knowledge_base = {
    "Какие еще у вас есть курсы?": "Данную информацию Вы можете уточнить у менеджеров учебного центра по номерам: +375445465454, +375295465454. \nТак же с помощью команды /lead Вы можете записаться на курс 'Войти в ИТ', на котором Вы сможете лучше узнать обо всех направлениях, получить индивидуальную консультацию, а также определиться с курсом.",
    ...
    "Стоимость курса?": "Курс Войти в IT, стоит 50 руб.",
...
async def collect_data(message: Message):
    ...
    if chat_id in collecting_data:
        ...
        elif 'first_name' not in user_data[chat_id]:
            user_data[chat_id]['first_name'] = message.text
            await message.reply("Введите ваше отчество:")
        elif 'middle_name' not in user_data[chat_id]:
            user_data[chat_id]['middle_name'] = message.text
            await message.reply("Введите ваш контактный телефон:")
        elif 'phone' not in user_data[chat_id]:
            user_data[chat_id]['phone'] = message.text
            await message.reply("Введите вашу электронную почту:")
    ...
    if best_match:
        ...
        if ("заявк" in user_input.lower()
                or "купить" in user_input.lower()
                or "приобрест" in user_input.lower()
                or "записат" in user_input.lower()):
            response = "\nДля оформления заявки на курс выберите команду /lead"
...
async def send_to_bitrix24(chat_id):
    ...
    if lead_data:
        params = {
            'fields': {
                'TITLE': f"Заявка от клиента: {chat_id}",
